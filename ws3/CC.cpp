...
void CC::aloCopy(const char *cc_name) {
    ...
    int length = strlen(cc_name); // FIXED: was strlen(name)
...
bool CC::validate(const char *name, unsigned long long cardNo, short cvv,
    ...
    bool valid = true;
    // name: should not be null and should be more than 2 characters
    ...
    // cvv: a three-digit number (100-999)
    if (cvv < 100 || cvv > 999) {
        valid = false;
    }
...
void CC::display(const char *name, unsigned long long number, short expYear,
    ...
    char lname[31]{};
    strncpy(lname, name, 30); // FIXED: was strcpy(lname, name, 30)
...
CC::CC(const char *cc_name, unsigned long long cc_no, short cc_cvv,
    ...
    if (validate(cc_name, cc_no, cc_cvv, cc_expMon,
                 cc_expYear)) { // FIXED: was using member variables
        aloCopy(cc_name);
        cardNo = cc_no;
        cvv = cc_cvv;
        expMon = cc_expMon;
...
bool CC::isEmpty() const {
    return name ==
           nullptr; // Returns true if cardholder name pointer is nullptr
}

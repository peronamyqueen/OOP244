...
int main() {
  ...
  cout << "Attempting to withdraw too much from account #4 using -= operator:"
       << endl;
  displayAccount(mixed[3] -= 5555.55);
  cout << "Attempting to deposit and withdraw negative amounts on account #4:"
       << endl;
  ...
  cout << value << endl;
  cout << "Attempting to change the account number of the valid account#7:"
       << endl;
  cout << "Before: " << endl;
  ...
  cout << "Total of " << countNewAccounts(mixed, MixedSize) << " new accounts"
       << endl;
  cout << "Total of " << countBadAccounts(mixed, MixedSize)
       << " invalid accounts" << endl;
  return 0;
}
...
void displayAccounts(const Account *acc, int num, int startFrom) {
  cout << "+---+-----+--------------------------------+-------+--------------+"
       << endl;
  cout << "|ROW|Init | Name                           | ACC#  |    BALANCE   |"
       << endl;
  cout << "+---+-----+--------------------------------+-------+--------------+"
       << endl;
  num += startFrom;
...
void displayAccount(const Account &acc) { displayAccounts(&acc, 1); }
void displayRawAccountNumber(const Account *acc, int num) {
